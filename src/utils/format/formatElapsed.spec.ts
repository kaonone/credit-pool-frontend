...
describe('formatElapsed', (): void => {
  ...
  it('formats a Date', (): void => {
    expect(formatElapsed(now, new Date(start + 9700))).toEqual('9.7s');
  });

  it('formats a BN', (): void => {
    expect(formatElapsed(now, new BN(start + 42700))).toEqual('42.7s');
  });
  ...
  it('formats a number', (): void => {
    expect(formatElapsed(now, start + 42 * 60 * 60000)).toEqual('42h');
  });

  it('formats defaults', (): void => {
    expect(formatElapsed()).toEqual('0.0s');
  });
});
